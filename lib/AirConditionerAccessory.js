    ...
    _registerCharacteristics(dps) {
        ...
        if (!this.device.context.noCool) {
            characteristicCoolingThresholdTemperature = service.getCharacteristic(Characteristic.CoolingThresholdTemperature)
                .setProps({
                    ...
                    maxValue: this.device.context.maxTemperature || 29,
                    minStep: this.device.context.minTemperatureSteps || 0.5
                })
                .updateValue(dps['2']/10)
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty('2')) {
                if (!this.device.context.noCool && characteristicCoolingThresholdTemperature && characteristicCoolingThresholdTemperature.value !== changes['2']/10)
                    characteristicCoolingThresholdTemperature.updateValue(changes['2']*10);
                if (!this.device.context.noHeat && characteristicHeatingThresholdTemperature && characteristicHeatingThresholdTemperature.value !== changes['2']/10)
                    characteristicHeatingThresholdTemperature.updateValue(changes['2']*10);
            }
